...
-- One news item per leader per day (prevents duplicates on re-run)
CREATE UNIQUE INDEX IF NOT EXISTS idx_news_leader_date
  ON news_events (leader, news_date);

-- Fast lookup: "give me today's news" or "give me last 7 days"
...
CREATE TABLE IF NOT EXISTS campaigns (
  ...
  end_time                TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '4 days'),
