...
import os from 'os';
import { generateTalkingHead, testReplicateConnection } from './replicateService.js';
...
export async function generateSubtitleReel(
    ...
    // Declared here so they are accessible in the finally block for cleanup
    let reporterTalkingVideoPath: string | null = null;
    let modiTalkingVideoPath: string | null = null;
    let isUsingReplicate = false;
    let perTurnVideoPaths: (string | null)[] = [];
    ...
    try {
        ...
        // Lip-sync is now handled by Replicate SadTalker AI — no local analysis needed
        console.log('[ffmpeg-subtitle] Lip-sync will be generated by Replicate SadTalker (per-turn).');
        ...
        // ── GENERATE AI TALKING HEADS (REPLICATE — PER-TURN) ─────────────
        // Each dialogue turn gets its own SadTalker call for precise lip-sync.
        // This is cheaper than full-length (~$0.15/reel vs $0.30) and produces
        // better quality since each clip is shorter.
        // ──────────────────────────────────────────────────────────────────────
        perTurnVideoPaths = new Array(turns.length).fill(null);
        reporterTalkingVideoPath = null;
        modiTalkingVideoPath = null;
        isUsingReplicate = false;

        if (process.env.REPLICATE_API_TOKEN) {
            const tokenPreview = process.env.REPLICATE_API_TOKEN.slice(0, 6) + '...' + process.env.REPLICATE_API_TOKEN.slice(-4);
            console.log(`[ffmpeg-subtitle] REPLICATE_API_TOKEN is set (${tokenPreview}). Provider: ${process.env.LIP_SYNC_PROVIDER || 'sadtalker'}`);
            
            // Quick connection test first
            const isConnected = await testReplicateConnection();
            if (!isConnected) {
                console.error('[ffmpeg-subtitle] ❌ Replicate connection test FAILED — token may be invalid or expired.');
                ...
                try {
                    ...
                    console.log(`[ffmpeg-subtitle] ✅ All ${turns.length} talking head videos generated in ${totalElapsed}s`);

                } catch (err: any) {
                    console.error('[ffmpeg-subtitle] ❌ Replicate per-turn generation failed:', err.message);
                    if (err.stack) console.error('[ffmpeg-subtitle]   Stack:', err.stack.split('\n').slice(0, 3).join('\n'));
                    console.log('[ffmpeg-subtitle] Continuing with static avatars (no lip-sync).');
                    // Clean up any partial results
                    for (const vp of perTurnVideoPaths) {
                        if (vp) { try { fs.unlinkSync(vp); } catch {} }
                    }
                    perTurnVideoPaths.fill(null);
                    isUsingReplicate = false;
                }
            }
        } else {
            console.log('[ffmpeg-subtitle] REPLICATE_API_TOKEN is not set. Using static avatars (no lip-sync).');
        }
        ...
        // Avatar / per-turn video inputs:
        let repIdx = -1;
        let modi1Idx = -1;
        let modi2Idx = -1;
        let modi3Idx = -1;
        const perTurnInputIndices: number[] = [];

        let nextInputIdx = hasBgAudio ? 3 : 2;

        if (isUsingReplicate) {
            // Per-turn Replicate mode: add each turn's talking-head video as a separate input
            for (let i = 0; i < turns.length; i++) {
                const videoPath = perTurnVideoPaths[i];
                if (videoPath && fs.existsSync(videoPath)) {
                    cmd.push('-i', videoPath);
                    perTurnInputIndices.push(nextInputIdx++);
                } else {
                    perTurnInputIndices.push(-1);
                }
            }
            console.log(`[ffmpeg-subtitle] Added ${perTurnInputIndices.filter(i => i !== -1).length} per-turn video inputs`);
        } else {
            // Static PNG fallback mode (no Replicate)
            if (fs.existsSync(reporterAvatar)) {
        ...
        if (isUsingReplicate) {
            // ── PER-TURN REPLICATE VIDEO OVERLAYS ──────────────────────
            // Each turn has its own SadTalker talking-head video.
            // Scale each to 1080px wide (full frame width) and overlay during the turn's time window.
            console.log('[ffmpeg-subtitle] Building per-turn Replicate video overlays...');

            for (let i = 0; i < turns.length; i++) {
                const inputIdx = perTurnInputIndices[i];
                ...
                // Scale video to fill width, position at bottom portion of frame
                const scaledPad = `[turn_${i}_scaled]`;
                overlayFilters.push(`[${inputIdx}:v] scale=1800:-1,format=rgba,setpts=PTS-STARTPTS ${scaledPad}`);

                const nextPad = `[v_overlay_${overlayCount++}]`;
                overlayFilters.push(`${currentPad}${scaledPad} overlay=x=-360:y=310:enable='${enableExpr}':shortest=1 ${nextPad}`);
                currentPad = nextPad;
            }

            // Show last speaker's avatar during outro (contextDuration)
            // Use the last turn's video frozen on last frame
            const lastTurnIdx = turns.length - 1;
            const lastInputIdx = perTurnInputIndices[lastTurnIdx];
            if (lastInputIdx !== -1) {
                const outroStart = turnEndTimes[lastTurnIdx];
                const outroEnd = fullDuration;
                const outroEnable = `between(t,${outroStart.toFixed(3)},${outroEnd.toFixed(3)})`;
                // Use last frame of last turn's video for outro
                const outroPad = `[outro_avatar]`;
                overlayFilters.push(`[${lastInputIdx}:v] scale=1800:-1,format=rgba,trim=end_frame=1,loop=-1:size=1,setpts=PTS-STARTPTS ${outroPad}`);
                const nextPad = `[v_overlay_${overlayCount++}]`;
                overlayFilters.push(`${currentPad}${outroPad} overlay=x=-360:y=310:enable='${outroEnable}':shortest=1 ${nextPad}`);
                currentPad = nextPad;
            }

            console.log(`[ffmpeg-subtitle] Added ${overlayCount} per-turn video overlays`);
        } else {
            // ── STATIC PNG FALLBACK (NO REPLICATE) ────────────────────
            // Static images shown during each speaker's turns (no lip-sync)
            console.log('[ffmpeg-subtitle] Using static PNG avatars (no lip-sync animation).');

            const getModiAvatarName = (turnIdx: number) => {
                let modiCount = 0;
                for (let i = 0; i <= turnIdx; i++) {
                    if (turns[i].speaker === 'modi') modiCount++;
                }
                if (turns[turnIdx].speaker === 'reporter') {
                    const lookaheadModiCount = modiCount + 1;
            ...
            const intervals: Record<string, string[]> = { 'rep': [], 'modi1': [], 'modi2': [], 'modi3': [] };

            for (let i = 0; i < turns.length; i++) {
                const intervalStr = `between(t,${turnStartTimes[i].toFixed(3)},${turnEndTimes[i].toFixed(3)})`;
                if (turns[i].speaker === 'reporter') {
                    intervals['rep'].push(intervalStr);
                } else {
                    intervals[getModiAvatarName(i)].push(intervalStr);
                }
            }
            // Add outro interval to last Modi avatar
            const getLastModiAvatarName = () => {
                let modiCount = 0;
                for (const t of turns) { if (t.speaker === 'modi') modiCount++; }
                if (modiCount >= 3) return 'modi3';
                if (modiCount === 2) return 'modi2';
                return 'modi1';
            };
            const lastModiName = getLastModiAvatarName();
            intervals[lastModiName].push(`between(t,${turnEndTimes[turns.length - 1].toFixed(3)},${fullDuration.toFixed(3)})`);

            const usedPads = new Set<string>();
            for (const [padName, list] of Object.entries(intervals)) {
                if (list.length > 0) usedPads.add(padName);
            }

            // Scale avatars
            const prepareAvatarPad = (name: string, inputIdx: number) => {
                if (inputIdx === -1 || !usedPads.has(name)) return;
        ...
        // Cleanup Replicate per-turn video files
        if (isUsingReplicate) {
            try {
                let cleanedCount = 0;
                for (const vp of perTurnVideoPaths) {
                ...
                console.log(`[ffmpeg-subtitle] Cleaned up ${cleanedCount} Replicate per-turn video files.`);
            } catch (err) {
                console.warn("[ffmpeg-subtitle] Failed to clean up Replicate temp videos:", err);
            }
        }
    }
}
