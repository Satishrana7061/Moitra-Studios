...
const RajneetiMap: React.FC = () => {
    ...
    return (
        <div className={`relative w-full min-h-[100dvh] bg-slate-950 select-none flex flex-col ${selectedState ? 'overflow-y-auto lg:overflow-hidden' : 'overflow-hidden'}`}>
            ...
            {/* LEFT SIDE NEWS PANEL — only shown when a state is clicked */}
            {selectedState && (
                <div className="absolute left-0 top-20 bottom-8 w-[280px] lg:w-[320px] z-50 hidden md:flex flex-col p-3 pointer-events-none" onClick={stopBubbling}>
                    <div className="pointer-events-auto bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden flex flex-col max-h-full shadow-2xl animate-card-slide">
                        {/* Panel Header */}
                        <div className="px-4 py-2.5 bg-gradient-to-r from-red-600 to-red-700 flex items-center gap-2 flex-shrink-0">
                            <Newspaper size={13} className="text-white" />
                            <span className="font-rajdhani font-black text-white tracking-widest text-[11px] uppercase truncate">
                                {selectedState} News
                            </span>
                            ...
                            {loadingStateNews ? (
                                ...
                                stateNews.map((news, idx) => {
                                    ...
                                    return (
                                        <button
                                            key={news.id || idx}
                                            onClick={() => navigate('/rajneeti-tv', { state: { activeIndex: idx, filterState: selectedState } })}
                                            className={`text-left group w-full p-2.5 rounded-xl border border-white/5 hover:border-white/20 ${sentBg} transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]`}
                                        >
                                ...
                                <div className="flex flex-col items-center justify-center py-8 text-center">
                                    ...
                                    <p className="text-slate-500 text-xs">No news available for<br/><span className="text-white font-semibold">{selectedState}</span></p>
                                </div>
                            )}
                        </div>
                    </div>
